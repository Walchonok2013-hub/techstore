...
#     class Meta:
#         model = get_user_model()  # правильно: вызываем функцию здесь, не импортируем внутри Meta
#         fields = ('username', 'email', 'first_name', 'last_name')
#         # password1 и password2 остаются автоматически из UserCreationForm — это поля для подтверждения пароля
