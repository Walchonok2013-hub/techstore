...
#     product = models.ForeignKey('catalog.Product', on_delete=models.CASCADE)  # Раскомментировано
...
class Address(models.Model):
    ...
    def __str__(self):
        return f'{self.street}, {self.city}, {self.country}'
class PaymentMethod(models.Model):
    # Вот это поле вызывает ошибку, если его нет в БД
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='payment_methods')
    ...
    def __str__(self):
        return f"{self.card_type} **** **** **** {self.last_digits}"
# class PaymentMethod(models.Model):
#     user = models.ForeignKey(User, on_delete=models.CASCADE)
#     last_digits = models.CharField(max_length=4)
#     is_default = models.BooleanField(default=False)    
